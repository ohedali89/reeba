...
var header = {
  init: function(){
    ...
    $('#menu1').slicknav({
      ...
      prependTo: '#mobile_nav1',
      label: '<div aria-haspopup="true" role="navigation" aria-label="Menu"><span class="icon-menu"></span> </div>',
      closedSymbol: '<span class="icon-right-arrow" aria-haspopup="true" aria-expanded="false"></span>',
      openedSymbol: '<span class="icon-down-arrow" aria-haspopup="true" aria-expanded="true"></span>',
      allowParentLinks: true,
      nestedParentLinks: false,
  ...
  init: function(){
    ...
    $('.product_slider').flexslider({
      ...
      start: function(slider){
        ...
        $(slider).find('img.gl-lazy:eq(0)').each(function() {
          var src = $(this).attr('data-src');
          $(this).attr('src', src).addClass('gl-lazyloaded').removeClass('gl-lazy');
        });
      },
      before: function (slider) {
          ...
          $('.product_slider .slides li:eq('+index+') img.gl-lazy').attr('src', $('.product_slider li:eq('+index+') img.gl-lazy').attr("data-src") ).addClass('gl-lazyloaded').removeClass('gl-lazy').css("opacity", "1");
	   }
    });
    ...
    /*$('#carouselVariation').flexslider({
        ...
        //slideshow: false,
        itemMargin: 5,
        asNavFor: '#productVariationslider'
      });

      $('#productVariationslider').flexslider({
        startAt: parseInt($('.featured_image', $(this)).data('index'), 10),
        touch: true,
        pauseOnHover: true,
        //controlNav: thumbnailPosition == "no-thumbnails" ? false : "thumbnails",
	...
	$('#productOriginalslider').flexslider({
      startAt: parseInt($('.featured_image', $(this)).data('index'), 10),
      touch: true,
      pauseOnHover: true,
      controlNav: thumbnailPosition == "no-thumbnails" ? false : "thumbnails",
	...
	*/
    ...
    $notify_form.each(function() {
      ...
      $('form.product_form').addClass('is-visible');
    ...
    // hide disabled button on initial pageload
    if ( $('a.action_button.klaviyo-bis-trigger').text().indexOf('Notify Me When Available') >= 0 ) {
      $('button#product-add-to-cart.disabled').hide();
    }

    // PDP accordion
    $('.product-details-accordion h5').on('click', function(){
      if ($(this).hasClass('accordion-open')) {
        // already open
        $(this).removeClass('accordion-open').next('div').slideUp();
      } else {
        $('.product-details-accordion h5').removeClass('accordion-open').next('div').slideUp();
        $(this).addClass('accordion-open').next('div').slideDown();
      }
    });

    // jump down to accordion for size table
    $('#jump-size-chart').on('click', function(){
      $('div.product-details-accordion > h5:nth-child(3)').click();
      $('html,body').animate({scrollTop: $('div.product-details-accordion > h5:nth-child(3)').offset().top},'slow');
    });

    // make left col on size table sticky
    $('div.size-chart > table > thead > tr > th:nth-child(1), div.size-chart > table > tbody > tr > td:nth-child(1)').addClass('size-table-col1');
    $('div.size-chart > table > thead > tr > th:nth-child(2), div.size-chart > table > tbody > tr > td:nth-child(2)').addClass('size-table-col2');
...
var map = {
  init: function(){
    if ($(".map-section .map, .contact-section .map, .details-section .map").hasClass('js-api-map')) {
      var mapsToLoad = [];
...
var collection = {
  init: function(){
      ...
      $('.collection_swatches_url span').on('click', function(){
        // swap image
        $('#bc-sf-filter-products img.secondary')
          .parent('.sc-badge-container')
          .remove();

        var swatch_img1 = $(this).attr('data-var-image1-url');
        var swatch_img2 = $(this).attr('data-var-image2-url');
        $(this).closest('div.thumbnail')
          .find('img.primary')
          .attr('src', swatch_img1)
          .attr('data-src', swatch_img1)
          .attr('data-srcset', swatch_img1 + ' 600w')
          .attr('srcset', swatch_img1 + ' 600w');

        $(this).closest('div.thumbnail')
          .find('img.secondary')
          .attr('src', swatch_img2)
          .attr('data-src', swatch_img2)
          .attr('data-srcset', swatch_img2 + ' 600w')
          .attr('srcset', swatch_img2 + ' 600w');

        // swap title
        $(this).closest('div.thumbnail')
          .find('.title')
          .text($(this).attr('data-var-title'));

        // swap url
        $(this).closest('div.thumbnail')
          .find('a')
          .attr('href', $(this).attr('data-var-url'));

        // swap price
        $(this).closest('div.thumbnail')
          .find('.price')
          .empty();
        if ( $(this).attr('data-var-price-compare') == 'Sold Out' ) {
          $(this).closest('div.thumbnail')
            .find('.price')
            .append('<span class="sold_out">Sold Out</span>');
        } else if ( $(this).attr('data-var-price-compare').length > 2 ) {
          $(this).closest('div.thumbnail')
            .find('.price')
            .append('<span class="money">' + $(this).attr('data-var-price') + ' </span><span class="was_price"><span class="money">' + $(this).attr('data-var-price-compare') + '</span></span>');
        } else {
          $(this).closest('div.thumbnail')
            .find('.price')
            .append('<span class="money">' + $(this).attr('data-var-price') + '</span>');
        }

        // swap badge
        var var_badge = $(this).attr('data-var-badge');

        if ( $(this).closest('div.thumbnail').find('.sc-badge-img').length <= 0 ) {
          $('.sc-badge-container').prepend('<div class="sc-badge-img top_right" style="background-size: cover; width: 100px; height: 100px;);"></div>');
        }

        if ( var_badge == 'Sale Badge' ) {
          $(this).closest('div.thumbnail')
            .find('.sc-badge-img')
            .css('background-image', "url('/apps/product-badge/uploads/002260/on-sale-badge2.png')");
        } else if ( var_badge == 'New Color Badge' ) {
        } else if ( var_badge == 'As-Is Badge' ) {
        } else if ( var_badge == 'Final Sale Badge' ) {
          $(this).closest('div.thumbnail')
            .find('.sc-badge-img')
            .css('background-image', "url('/apps/product-badge/uploads/002260/final-sale-badge.png')");
        } else if ( var_badge == 'New Relist Badge' ) {
        } else if ( var_badge == 'New Item Badge' ) {
          $(this).closest('div.thumbnail')
            .find('.sc-badge-img')
            .css('background-image', "url('/apps/product-badge/uploads/002260/new-item_copy.png')");
        } else {
          $(this).closest('div.thumbnail')
            .find('.sc-badge-img')
      ...
      $.ajax({
        ...
        success: function(item) {
          ...
          window.setTimeout(function(){
      	  ...
      	  $('body.product').removeClass('drawer-cart-open');
          }, 1000);
  ...
  if (variant && variant.available == true) {
    ...
    if (variant.inventory_management && variant.inventory_quantity > 0) {
        ...
        if(variant.inventory_policy == "deny") {
          $('.quantity', $product).attr('data-max', variant.inventory_quantity);
        }
    ...
    if (variant.price > 0) {
      console.log(Shopify.formatMoney(variant.price, $productForm.data('money-format')));
      console.log(main_percentage_discount);

      // 3/12/2020 - ccowden: I commented this block of code because it was causing a display error for select products.
      // 			Let's understand why this appears to be duplicated in product-template.liquid in the code blocks
      //			around lines 86 and 92.
       if(main_percentage_discount !=1 ){
        $('.current_price', $product).html('<span class="money">' + Shopify.formatMoney(variant.price*main_percentage_discount, $productForm.data('money-format')) + '</span>');
    ...
    $('.cstmKlaviyo').hide();
    $('.add_to_cart').show();
    $('.sold_out', $product).html('');
  } else {
    //var message = variant ? "Sold Out" : "This combination is unavailable";
    ...
    $notify_form.fadeIn();
    if(product_tags.includes("restock")){
...
$(document)
  .on('shopify:section:load', function(e){
    ...
    if ($parentSection.hasClass('map-section') || $parentSection.hasClass('contact-section') || $parentSection.hasClass('details-section')) {
      map.init();
    }
...
$(document).ready(function(){
  var sliderMainInitialized = false;
  var sliderThumbInitialized = false;
  var $sliderProductMain = $('.product-photo__main__photos');
  var $sliderProductThumb = $('.product-photo__thumbs');
  var isMobileView = window.matchMedia("(max-width: 769px)").matches;

  function destroyProductSlider() {
    if ($sliderProductMain && sliderMainInitialized) {
      $sliderProductMain.slick('unslick');
      sliderMainInitialized = false;
    }

    if ($sliderProductThumb && sliderThumbInitialized) {
      $sliderProductThumb.slick('unslick');
      sliderThumbInitialized = false;
    }
  ...
  function initProductSlider() {
    ...
    //var mediaQ = window.matchMedia("(max-width: 769px)")
    if (isMobileView) { // mobile
      ...
      sliderMainInitialized = true;
    } else { // desktop
      $sliderProductMain.slick({
        arrows: false,
        asNavFor: '.product-photo__thumbs',
        adaptiveHeight: false,
        dots: false,
        infinite: false,
        fade: true
      });
      $sliderProductThumb.slick({
        asNavFor: '.product-photo__main__photos',
        slidesToShow: 5,
        slidesToScroll: 1,
        arrows: true,
        dots: false,
  ...
  $(".main-nav__list-item.dropdown").hover(function() {
    var $menu = $(this);
    $(".main-nav__list-item.dropdown").removeClass("is-active");
    $menu.addClass("is-active");

    setTimeout(function() {
      var windowWidth = $(window).width();
      var panelWidth = $menu.find(".main-nav__dd-panel").width();
      var panelPosEnd = panelWidth + $menu.find(".main-nav__dd-panel").offset().left;

      if (panelPosEnd > windowWidth) {
        if (panelWidth > windowWidth) {
          $menu.find(".main-nav__dd-panel").css({
            "left": "-" + ($menu.find(".main-nav__dd-panel").offset().left - 10) + "px",
          });
          $menu.find(".main-nav__dd-panel-inner").css({
            "overflow-x": "auto",
            "width": (windowWidth - 20 - 34) + "px" //34 is padding width
          });
        } else if (panelWidth > (windowWidth / 4 * 2)) {
          var pos = ((windowWidth - panelWidth) / 2) - $menu.find(".main-nav__dd-panel").offset().left;
          $menu.find(".main-nav__dd-panel").css("left", pos + "px");
        } else {
          var pos = windowWidth - panelPosEnd - 10;
          $menu.find(".main-nav__dd-panel").css("left", pos + "px");
        }
      }
    }, 110);
  ...
  $(window).resize(function(){
  	$(".main-nav__dd-panel, .main-nav__dd-panel-inner").removeAttr("style");

    var mediaQ = window.matchMedia("(max-width: 769px)");
    if (mediaQ.matches && !isMobileView) { // mobile
      isMobileView = true;
      initProductSlider();
    } else if (!mediaQ.matches && isMobileView) { // desktop
      isMobileView = false;
      initProductSlider();
    }
  });
  ...
  initProductSlider();
/*
  $(".main-nav__list-item.dropdown").on("click", function(e) {
    e.preventDefault();
    var $menu = $(this);
  ...
  $("body").on("click", function(event) {
    if (!$(event.target).is(".main-nav__list-item.dropdown") && !$(event.target).is(".main-nav__link") && $(".main-nav__list-item.is-active").length) {
      $(".main-nav__list-item.is-active").removeClass("is-active");
    }
  });*/
})
...
window.onload = function(){
  ...
  $('.search_form_get').on('click', function(){
  	$(this).hide();
    $('.search_form_close').show();
    $('#desktop_search').slideToggle("fast");
    $('#bc-sf-search-box-1').focus();
    //$('#headerOriginal .search_icon .search').show();
  });
  $('.search_form_close').on('click', function(){
  	$(this).hide();
    $('.search_form_get').show();
    $('#desktop_search').slideToggle("fast");
    //$('#headerOriginal .search_icon .search').slideToggle("slow")
    //$('#headerOriginal .search_icon .search').show();
  });
  ...
  $.get("https://freegeoip.app/json/", function (response) {
    ...
    {
      $(".free_shipping").remove();
    }
  });

};

/* Boost - Product Filter & Search App helper */
$(window).on('load', function(){
  if ($('body[class^="collection-"]').length >= 0) {
    $('.boost-pfs-filter-products div[data-product-id] .new_banner').each(function(){
      if ( $(this).text().indexOf('Coming Soon!') >= 0 ) {
        $(this).prev('.info').find('.price').remove();
      }
    });
  }
});
